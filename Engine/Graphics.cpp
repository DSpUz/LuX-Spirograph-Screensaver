...
Graphics::Graphics( HWNDKey& key, int ScWidth, int ScHeight)
	:
	ScreenWidth(ScWidth),
...
void Graphics::InitializeScreenDimensions()
{
...
void Graphics::DrawLine(int x1, int y1, int x2, int y2, Color col)
{
	...
	if (dy == 0 && dx == 0)
	{
		if (x1 >= 0 && x1 < Graphics::ScreenWidth&&y1 >= 0 && y1 < Graphics::ScreenHeight) {
			PutPixel(x1, y1, col);
		}
	}
